...
(function () {
  ...
  /* Един и същ (вече активиран) адрес в два вида ендпойнт:
     AJAX — за заявки без прикачен файл, остава на страницата;
     нативен POST — когато има аудио, защото FormSubmit приема прикачени
     файлове само през multipart форма, не през AJAX.
     Щом намериш случайния низ от активационния имейл, сложи него в
     FORM_TARGET — пощата ти изчезва от публичния JS, ендпойнтите не се
     променят. */
  ...
  var POVOD_MAP = {
    ...
    /* Псевдоними за SEO страниците pesen-za-*.html — сочат към същите
       чипове, само с ключ, който съвпада с името на съответната страница.
       07.09.2026: „Ергенско / Моминско“ и „Бебе / Кръщене“ са вече по два чипа. */
    "mominsko": "Моминско",
    "jubilej": "Юбилей",
    "bebeski": "Бебе",
    /* Вълна А по pesenta-flags/povodi-razshirenie.md (07.09.2026): ключът е
       slug-ът на страницата podarak-pesen-za-<slug>.html. */
    "krashtene": "Кръщене",
    "pensionirane": "Пенсиониране",
    "godej": "Годеж",
    "abiturientski-bal": "Абитуриентски бал",
    "imen-den": "Имен ден",
    "mama": "За мама",
    "palnoletie": "Пълнолетие",
    /* Вълна Б */
    "diplomirane": "Дипломиране",
    "nov-dom": "Нов дом",
    "uchitel": "За учител",
    "kumove": "За кумовете",
    "lovec": "За ловец",
  ...
  function validateStep(step) {
    if (step === 3) {
      if (!val("cust-name")) {
        return { msg: "Напиши името си — трябва ни за фактурата.", field: "cust-name" };
      }
      var email = val("cust-email");
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email)) {
        return { msg: "Въведи валиден имейл — там ще получиш песента.", field: "cust-email" };
      }
  ...
  function rememberOrder(orderNo, planLabel, totalStr) {
    try {
