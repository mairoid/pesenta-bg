  ...
  (function initFastVoice() {
    ...
    function startRec() {
      ...
      navigator.mediaDevices.getUserMedia({ audio: true }).then(function (s) {
        ...
        /* диктовката тръгва заедно със записа — клиентът вижда текста веднага
           и може да оправи имената, преди да натисне „Изпрати“ */
        trWrap.hidden = false;
    ...
    sendBtn.addEventListener("click", function () {
      ...
      var ext = audioExt(blob);
      var fileName = "glasovo-" + orderNo + "." + ext;
      ...
      fields["Транскрипция"] = transcript || "— няма (виж записа)";

      /* заявката се записва ПРЕДИ изпращането: нативният POST сменя страницата */
      rememberOrder(orderNo, "Гласова поръчка", "—");
