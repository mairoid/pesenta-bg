 ...
 * зависи от това дали 2,5 MB видео са пристигнали.
 */
(function () {
  "use strict";
  ...
  function tick() {
    ...
    /* Seek само при реална разлика и само ако предишният е приключил.
       Половин кадър (1/48 сек) е прагът, под който няма какво да се види. */
    if (!seeking && Math.abs(current - vid.currentTime) > 1 / 48) {
      seeking = true;
      try { vid.currentTime = current; } catch (e) { seeking = false; }
    }
  ...
  function zaredi() {
    ...
    var chasovnik = setTimeout(function () {
      /* Прекратяваме тегленето: иначе 2,5 MB продължават да ядат от
    ...
    /* БЕЗ cache: "force-cache". Той значи „ползвай кешираното, дори да е
       ...
       живо: кешът връщаше 2483 KB / 8 сек, а сървърът 1480 KB / 4,4 сек.
