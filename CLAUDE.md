...
- Стил: `assets/css/style.css` (CSS variables в `:root`). JS по назначение: `main.js` (плейър, hero видео, навигация), `order.js` (съветникът в poruchka.html), `voice-order.js` и `text-order.js` (бързите поръчки в hero-а — гласовата върви и на poruchka.html), `payments.js` (Stripe Payment Links).
